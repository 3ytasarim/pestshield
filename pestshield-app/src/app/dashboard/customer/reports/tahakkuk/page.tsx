...
const INITIAL_APPLIED: Applied = { startDate: "", endDate: "", durum: "all" };

export default function CustomerPortalTahakkukPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/portal/reports/tahakkuk?${params.toString()}`)
  ...
  function handleListele() {
    setApplied({ startDate, endDate, durum });
  }
  ...
  async function handleOpenBelge(row: TahakkukRow) {
    ...
    try {
      await printEk1Form(row.ek1Form, { periodDate: row.periodDate, startTime: row.startTime, endTime: row.endTime }, row.customerName, row.batchName || row.serviceName);
    } finally {
      setOpeningId(null);
    }
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-1.5">
        <h1 className="text-[1.75rem] leading-tight font-semibold tracking-tight text-foreground">Tahakkuk Raporları</h1>
        <p className="text-sm text-muted-foreground">Periyot ziyaretlerinizin tahakkuk (hakediş) durumu — tamamlanan ve bekleyen kayıtlar.</p>
      </div>
      ...
      <Card className="min-w-0 rounded-2xl border-border/60 shadow-sm">
        ...
        <CardContent className="grid grid-cols-1 items-end gap-4 pt-4 sm:grid-cols-2 xl:grid-cols-4">
          <div>
          ...
          <Card className="min-w-0 gap-0 overflow-hidden rounded-2xl border-border/60 py-0 shadow-sm">
            <CardHeader className="flex-row items-center justify-between gap-2 border-b border-border/60 bg-muted/30 px-4 py-3.5">
              ...
              <div className="relative w-full max-w-[220px]">
                ...
                <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Hizmet ara…" className="h-9 rounded-lg pl-8 text-xs" />
              </div>
            </CardHeader>
