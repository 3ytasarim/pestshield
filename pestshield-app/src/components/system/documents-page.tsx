...
export function DocumentsPage() {
  ...
  async function handleSave() {
    ...
    if (!file) {
      toast.error("Bir PDF dosyası seçin");
      return;
    }
    setSaving(true);
    try {
  ...
  function handleView(doc: CompanyDocument) {
            ...
            <TabsContent value="listele" className="mt-4">
              <div className="flex flex-col gap-4">
                ...
                {loading ? null : filtered.length === 0 ? (
                  <EmptyState icon={FileText} title="Henüz belge yok" description="EKLE sekmesinden yeni bir belge yükleyin." />
                ) : (
                  <>
                    <Table>
                      <TableHeader>
                      ...
                      <TableBody>
                        {pageItems.map((doc, i) => (
                          <TableRow key={doc.id} className="cursor-pointer" onClick={() => handleView(doc)}>
                            ...
                            <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                              <div className="flex items-center justify-end gap-1.5">
                                <Button size="icon-sm" variant="outline" onClick={() => handleView(doc)} aria-label="Belgeyi görüntüle">
